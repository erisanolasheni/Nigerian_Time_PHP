...
public function getMinute($minute)
{
	...
	if ($minute > 30) $description = IgboConstants::BEFORE_POINTER . $this->getNumber(60 - $minute);
	elseif ($minute < 30) $description = IgboConstants::AFTER_POINTER . $this->getNumber($minute);

	return $description;
}
public function getHour($hour)
{
	if ($hour > 12) return IgboConstants::TIME_POINTER . $this->getNumber($hour-12);
	else return IgboConstants::TIME_POINTER . $this->getNumber($hour);
	}
